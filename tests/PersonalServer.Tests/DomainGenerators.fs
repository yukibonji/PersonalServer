...
//https://msdn.microsoft.com/en-us/library/system.char.iswhitespace(v=vs.110).aspx
    ...
    let genFullName() =
        gen { 
                ...
                return
                    FullName.TryParse (first, middle, family, NameOrder.Western, Set.empty<Tag>, Sources.sourceSet)
        }
    ...
    let genNameAndAffixes() =
        gen { 
                ...
                return
                    NameAndAffixes.TryParse (salutations, simpleName.ToString(), suffixes, Set.empty<Tag>, Sources.sourceSet)
        }
        |> Gen.filter Option.isSome
        |> Gen.map (fun x -> x.Value)
    ...
    let genPhysicalAddress() =
        gen { 
                ...
                return
                    PhysicalAddress.TryParse (streetAddress, city, state, postalCode, country, Set.empty<Tag>, Sources.sourceSet)
        }
        |> Gen.filter Option.isSome
        |> Gen.map (fun x -> x.Value)
